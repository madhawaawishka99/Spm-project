...
class packageObj extends StatefulWidget {
  const packageObj({super.key});

  @override
  State<packageObj> createState() => _packageObjState();
}

class _packageObjState extends State<packageObj> {
  ...
  Future<void> _tfliteInit() async {
    ...
    if (res == null) {
      devtools.log("Failed to load the package model");
      return;
    }
    devtools.log("package Model loaded successfully");
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("PACKAGE OBJECTS"),
        centerTitle: true,
